...
#         subprocess.run(["bash", "-c", "source ~/miniconda/etc/profile.d/conda.sh"], check=True, shell=True)
#         # Activate the environment
#         subprocess.run(["bash", "-c", f"conda activate {env_name}"], check=True, shell=True)
#         click.echo(f"Environment '{env_name}' activated successfully.")
#     except subprocess.CalledProcessError as e:
#         click.echo(f"Error: {e}")

# Function to manually activate the selected Conda environment
def activate_env(env_name):
    click.echo(f"Activating environment '{env_name}' manually...")
    env_bin_path = os.path.expanduser(f"~/miniconda/envs/{env_name}/bin")
    activate_script = os.path.join(env_bin_path, "activate")
    if os.path.exists(activate_script):
        try:
            subprocess.run([f"source {activate_script}"], check=True, shell=True)
            click.echo(f"Environment '{env_name}' activated successfully.")
        except subprocess.CalledProcessError as e:
            click.echo(f"Error: {e}")
    else:
        click.echo(f"Error: Activation script for environment '{env_name}' not found.")
